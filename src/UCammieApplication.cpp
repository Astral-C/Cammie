...
UCammieApplication::UCammieApplication() {
	mWindow = nullptr;
	mContext = nullptr;
}

bool UCammieApplication::Setup() {
	// Initialize GLFW
	if (!glfwInit())
		return false;
	...
	mContext = new UCammieContext();


	return true;
}

bool UCammieApplication::Teardown() {
...
bool UCammieApplication::Execute(float deltaTime) {
